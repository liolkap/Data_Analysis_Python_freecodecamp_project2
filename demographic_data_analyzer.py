...
def calculate_demographic_data(print_data=True):
    ...
    # How many of each race are represented in this dataset? This should be a Pandas series with race
    ...
    higher_education = ed['Bachelors'] + ed['Masters'] + ed['Doctorate']
    lower_education = ed.sum() - higher_education
    ...
    highest_earning_country = (df[df.salary == '>50K']['native-country'].value_counts() / df[
        'native-country'].value_counts() * 100).round(1)
    highest_earning_country = highest_earning_country.sort_values(ascending=False).head(1)
    highest_earning_country_percentage = highest_earning_country.iloc[0]
    highest_earning_country = highest_earning_country.index.tolist()[0]

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df.loc[(df['native-country'] == 'India') & (df['salary'] == '>50K')][
        'occupation'].value_counts()
    top_IN_occupation = top_IN_occupation.sort_values(ascending=False).head(1).index.tolist()[0]
